...
/*
 ...
 Как только я запускаю приложение, анимация работает нормально
 (она длится 1 секунду (положим, я прерываю ее через секунду). Все хорошо)
 Через какое-то время, после того как я похожу по экранчикам, что-то поделаю, время ее жизни сокращается
 (она длится, скажем, 0.5 сек)
 Еще через какое-то время, она начинает буквально мигать (появляется и тут же исчезает).
 Скажем, она работает 0.1 сек.
